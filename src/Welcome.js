...
export default class Welcome extends Component {
  ...
  addsPlayerNamesToWelcomeState = () => {
    let localStorageKeysArr = Object.keys(localStorage);
    this.setState({ playerNames: localStorageKeysArr });
  }

  addsPlayerNamesToReturningPlayerSelection = () => {
    this.returningPlayerNameSelectionScreenState();
    this.addsPlayerNamesToWelcomeState();
  }
    ...
    return (
      <div className='Welcome'>
        <div className={ this.state.welcomePage ? 'Welcome-one' : 'Welcome-one slide-out' }>
          ...
          <button className='multipurpose-btn' type='button' onClick={this.addsPlayerNamesToReturningPlayerSelection}> Returning Player</button>
        </div>
            ...
            <div className={ this.state.selectConfidence ? 'Welcome-two slide-out margin-remove' : this.state.welcomePage ? 'Welcome-two' : 'Welcome-two slide-in' }>
              {
                this.state.playerNames.map((name, key) => {
                  return <button className='multipurpose-btn' onClick={() => this.props.updateReturningPlayerName(name)} key={key}>{name}</button>
                })
              }
              <button className='new-player-input-back-btn' type='button' onClick={this.changeStateToPreviousState}><i className="fas fa-angle-left"></i></button>
            </div>
